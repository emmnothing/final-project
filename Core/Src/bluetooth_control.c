...
bool BluetoothControl_SendText(const char *text)
{
  ...
  if (HAL_UART_Transmit(&huart2, (uint8_t *)text, (uint16_t)length, 50U) != HAL_OK)
  {
    s_state.ready = false;
    return false;
  }
...
void BluetoothControl_OnUartRxCpltFromIsr(UART_HandleTypeDef *huart)
{
  ...
  if ((huart == NULL) || (huart->Instance != USART2) || (s_rx_line_queue == NULL))
  {
    return;
  }
...
static void BluetoothControl_SendHeartbeat(uint32_t now)
{
  ...
  if ((now - s_state.last_tx_tick_ms) < BLUETOOTH_HEARTBEAT_MS)
  {
    return;
  }
  ...
  if (s_state.debug_enabled)
  {
    (void)snprintf(
        line,
        sizeof(line),
        "DBG map=%lu last=%s rx=%lu err=%lu drop=%lu\r\n",
        s_state.mapping_active ? 1UL : 0UL,
        BluetoothControl_CommandName(s_state.last_command),
        (unsigned long)s_state.rx_lines,
        (unsigned long)(s_state.parse_errors + s_state.uart_errors),
        (unsigned long)s_state.command_drops);
